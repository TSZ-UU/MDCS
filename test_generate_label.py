...
@torch.no_grad()
def test(args, model, test_dataloader, epoch, model_name,save_dir):
    model.eval()
    val_dice = [0.0] * n_part
    ...
    for i in range(domain_num):
        ...
        for batch_num, sample in enumerate(cur_dataloader):
            ...
            # Display the image (this won't show up, just for saving)
            ax.imshow(pred_label_np[0], cmap='gray')
            ax.axis('off')  # Hide axes

            # Save the image to a file
            domain_save_dir = os.path.join(save_dir, f"domain_{domain_code}")
            if not os.path.exists(domain_save_dir):
                os.makedirs(domain_save_dir)

            # Generate a unique filename with timestamp
            timestamp = int(time.time() * 1000)
            save_path = os.path.join(domain_save_dir, f"pred_domain{domain_code}_{timestamp}.png")
    ...
    if args.eval:
        ...
        if args.model == 'unet':
            ...
            model.load_state_dict(torch.load(
                r'D:\Project\SynFoC-master\result\prostateSlice\unet_avg_dice_best_model.pth'))
        elif args.model == 'SAM':
            model, img_embedding_size = create_model(model_name='SAM')
            model.load_lora_parameters(
                r'D:\Project\SynFoC-master\result\prostateSlice\SAM_avg_dice_best_model.pth')
        test(args, model, test_dataloader, args.lb_domain, args.model,snapshot_path )
        exit()


if __name__ == "__main__":
    snapshot_path = "D:/Project/SynFoC-master/model/" + args.dataset + "/" + args.save_name + "/"
    if args.dataset == 'fundus':
        num_channels = 3
        num_classes = 2
    ...
    args.ckpt = 'D:\Project\SynFoC-master\code\checkpoints\medsam_vit_b.pth'
