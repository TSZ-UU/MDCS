...
class _LoRA_qkv(nn.Module):
    ...
    def forward(self, x):
        ...
        #将前dim列，对应query的矩阵部分，把new_q添加上去
        qkv[:, :, :, : self.dim] += new_q
        #将new_v添加到value矩阵上去
        qkv[:, :, :, -self.dim:] += new_v
        return qkv
...
class LoRA_Sam(nn.Module):
    ...
    def __init__(self, sam_model: Sam, r: int, lora_layer=None):
        ...
        assert r > 0
        # base_vit_dim = sam_model._image_encoder.patchembed.proj.out_channels
        # dim = base_vit_dim
        if lora_layer:
        ...
        for t_layer_i, blk in enumerate(sam_model.image_encoder.blocks):
            ...
            if t_layer_i not in self.lora_layer:
                continue
            #获得qkv矩阵，获取输入维度dim，重置q、v矩阵用A（dim，r）B（r，dim）
            ...
            #替换原有的 QKV 变换，用lora的A、B替换掉原来的QKV变换
